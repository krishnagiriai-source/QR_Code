...
/* ══════════════════════════════════════════════
   ADMIN PASSWORD GATE
   Password: MRKadmin!@123
   Uses sessionStorage — re-prompts on new tab/session
══════════════════════════════════════════════ */
const ADMIN_SESSION_KEY = 'mrk_admin_auth';
const ADMIN_PASSWORD    = 'MRKadmin!@123';
...
function checkAdminPassword() {
  ...
  if (entered === ADMIN_PASSWORD) {
    /* Correct — store session flag and unlock */
    ...
    if (gate) {
      ...
      gate.style.transform  = 'scale(.97)';
      setTimeout(() => { gate.classList.add('hidden'); document.body.classList.remove('admin-gate-active'); }, 360);
    }
