...
/* Auto-download managers.json on every change.
   Upload the downloaded file to GitHub → Vercel redeploys → customers see updates in ~30s */
function autoExportJSON() {
  var list = loadManagersLocal();
...
function toggleAvail(id) {
  ...
  autoExportJSON();
  showToast(mgr.manager+' → '+(mgr.available?'✅ Available':'⏳ Coming Soon')+'. managers.json auto-downloaded.','success',3500);
}
...
function _startAdmin() {
  /* Run actual admin setup — called only when authenticated */
  renderAdminList();
  wireAdminEvents();
  initAddPreview();
...
function initAdmin() {
  /* Password gate is handled by inline <script> in admin.html.
     By the time this runs, gate is either showing or already passed. */
  if (sessionStorage.getItem('mrk_auth') === '1') {
    ...
    /* Gate will be shown by the inline script in admin.html */
    /* Just hide the loader — gate handles the rest */
    hideLoader();
  }
}
